...
{
    float n = .19 * exp(-mod(u_time * 0.14, -3.)) - 1.2;// noise amount
    float iter = 2.0;
    for (int i = 0; i < 8; i++)
    {
...
float map(vec3 p)
{
    //    R(p.xz, iMouse.x*0.008*pi+u_time*0.1);
...
void main()
{
    ...
    vec3 rd = normalize(vec3((fragCoord.xy-0.5*u_resolution.xy)/u_resolution.y, 1.));
    ...
    if (RaySphereIntersect(ro, rd, min_dist, max_dist))
    {
        ...
        #endif
        {
            ...
            */
            ...
            uvd.y*=120.;
            uvd.x*=280.;
            d=abs(d)*(.8+0.08*texture(noise_texture2, vec2(uvd.y, -uvd.x+0.5*sin(4.*u_time+uvd.y*4.0))).r);
            #endif
    ...
    #else
    if (u_time > 20.0) {
        gl_FragColor = vec4(sum.xyz, 0.0);
    } else {
        gl_FragColor = vec4(sum.xyz, sum.x);
    }
    #endif
}
